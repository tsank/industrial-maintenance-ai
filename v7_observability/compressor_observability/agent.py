"""
agent.py — LangGraph StateGraph for the Industrial Maintenance AI v7.

New in v7 vs v6:
  - OTel tracing via @traced_node decorator on all node functions
  - Child spans on helper functions (_graph_search, _generate_sql, _execute_sql)
  - LangSmith tracing activated via environment variables in clients.py
  - generate_node and re_retrieve_node instrumented inline (defined in this file)
  - Jaeger receives all spans via OTLP gRPC exporter (localhost:4317)
...
Inherited from v6 (unchanged):
  ...
  - synthesise_node prepends retrieved memories to context
...
Observability coverage per node:
  ...
  node.retrieve_hybrid    sql_context_length, graph_context_length
    helper.graph_search   (same as above — automatic child span)
    helper.generate_sql   (same as above — automatic child span)
    helper.execute_sql    (same as above — automatic child span)
...
from compressor_observability.state import AgentState
from compressor_observability.nodes.memory_retrieval import memory_retrieval_node
from compressor_observability.nodes.memory_storage import memory_storage_node
from compressor_observability.nodes.classify import classify_node
from compressor_observability.nodes.retrieve_spec import db_retrieve_node
from compressor_observability.nodes.retrieve_vector import vector_retrieve_node
from compressor_observability.nodes.retrieve_graph import graph_retrieve_node
from compressor_observability.nodes.retrieve_hybrid import hybrid_retrieve_node
from compressor_observability.nodes.synthesise import synthesise_node
from compressor_observability.nodes.judge import judge_node
...
from compressor_observability.nodes.clients import llm
from compressor_observability.observability import traced_node

@traced_node("node.generate")
def generate_node(state, span) -> dict:
...
@traced_node("node.re_retrieve")
def re_retrieve_node(state, span) -> dict:
    targeted_query = (
    ...
    span.set_attribute("fallback_store", fallback_store)
    span.set_attribute("new_content_length", len(new_content))
